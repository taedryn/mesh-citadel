...
class RegisterUserWorkflow(Workflow):
    ...
    async def handle(self, processor, token, state, command, wf_state):
        ...
        # Step 1: Username
        if step == 1:
            username = command.args.get("response")
        ...
        # Step 2: Display Name
        if step == 2:
            display_name = command.args.get("response")
            if not display_name:
                return ErrorResponse(
        ...
        # Step 3: Password
        if step == 3:
            password = command.args.get("response")
            ...
            data["password_hash"] = hash_password(password, data["salt"])
            try:
        ...
        # Step 4: Terms
        if step == 4:
            agree = command.args.get("response", "").lower()
        ...
        # Step 5: Intro
        if step == 5:
            intro = command.args.get("response")
            data["intro"] = intro
            wf_state["step"] = 6
            return CommandResponse(
                success=True,                                                
        ...
        # Step 6: Finalize
        if step == 6:
            confirm = command.args.get("response", "").lower()
